...
def get_connection():
    """
    Get a pooled connection to the SQLite database.

    This now uses connection pooling for better performance.
...
def init_db():
    ...
    # Run migrations for feature-enhanced predictions (with version tracking)
    try:
        from api.utils.db_migrations import run_migrations
        run_migrations()
    except ImportError:
        try:
