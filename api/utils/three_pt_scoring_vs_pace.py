"""
Three-Point Scoring vs Pace Module

Aggregates team 3PT scoring performance by game pace tier and location.
Provides pace-adjusted home/away 3PT scoring splits for visualization and analysis.

This module queries team_game_logs and categorizes games by pace
...
def get_pace_tier(pace: float) -> Optional[str]:
    """
    Map game pace to tier classification.

    Args:
        pace: Game pace (possessions per 48 minutes)

    Returns:
        Tier name ('slow', 'normal', 'fast') or None if pace is missing/invalid
    """
    if pace is None:
        return None
...
def get_team_three_pt_scoring_vs_pace(team_id: int, season: str = '2025-26') -> Optional[Dict]:
    """
    Get pace-adjusted home/away 3PT scoring splits for a team.

    Aggregates team's 3PT scoring performance across 6 buckets:
    - Home in Slow pace games (<98)
    - Home in Normal pace games (98-102)
    - Home in Fast pace games (>102)
    - Away in Slow pace games (<98)
    - Away in Normal pace games (98-102)
    - Away in Fast pace games (>102)
    ...
    Returns:
        Dictionary with team info, season average 3PT PPG, and splits by pace tier/location.
    ...
    try:
        ...
        # Step 2: Fetch game logs with pace
        ...
        # Step 3: Aggregate games by pace tier and location
        ...
        for game in game_logs:
            pace = game['pace']
            tier = get_pace_tier(pace)

            # Skip games where pace tier can't be determined
            if tier is None:
                logger.debug(f"Skipping game with missing pace: {pace}")
                continue
        ...
        logger.info(f"Generated 3PT scoring vs pace splits for team {team_id} ({team_info['team_abbreviation']}) - {season}")
        return team_info

    except Exception as e:
        logger.error(f"Error generating 3PT scoring vs pace splits for team {team_id}: {e}")
        conn.close()
        return None
