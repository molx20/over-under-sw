/**
 * AssistsVsPaceChart Component
 *
 * Bar chart showing team assists by game pace tier and location.
...
function AssistsVsPaceChart({ teamData, compact = false }) {
  // Drilldown state
  ...
  if (!teamData || !teamData.splits) {
    return (
      <div className="text-center text-gray-500 dark:text-gray-400 py-8">
        No assists vs pace data available
      </div>
    )
  }

  const { splits, season_avg_ast, team_abbreviation, projected_pace } = teamData
    ...
    <div className={`${compact ? 'p-3' : 'p-3 sm:p-4 md:p-6'} bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 w-full max-sm:overflow-x-hidden`}>
              ...
              return (
                ...
                max-sm:right-0 sm:left-1/2 sm:transform sm:-translate-x-1/2">
                  ...
                  <div className="mt-2 sm:mt-3 text-center">
                    ...
                    {isProjectedPaceTier && projected_pace && (
      ...
      <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-700">
        <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 leading-relaxed">
          <strong>How to read:</strong> Shows {team_abbreviation}'s assists per game in different pace environments.
          Faster-paced games typically create more possessions and assist opportunities, while slower games may limit ball movement.
          {projected_pace && (
            <span className="block mt-2 text-yellow-600 dark:text-yellow-400 font-medium">
              Today's projected pace is <strong>{projected_pace.toFixed(1)}</strong> ({projectedPaceTier} pace).
            </span>
          )}
        </p>
      </div>
...
export default AssistsVsPaceChart
