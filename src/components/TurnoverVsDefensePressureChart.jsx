/**
 * TurnoverVsDefensePressureChart Component
 *
 * Bar chart showing team turnovers by opponent defensive pressure tier and location.
 * X-axis: Defensive pressure tiers (Elite, Average, Low)
...
function TurnoverVsDefensePressureChart({ teamData, compact = false }) {
  ...
  if (!teamData || !teamData.splits) {
    return (
      <div className="text-center text-gray-500 dark:text-gray-400 py-8">
        No turnover splits data available
      </div>
    )
  }
  ...
  const {
    ...
    team_abbreviation,
    opponent_turnover_pressure_tier,
    opponent_opp_tov_rank,
    opponent_def_rtg_rank
  } = teamData
    ...
    <div className={`${compact ? 'p-3' : 'p-3 sm:p-4 md:p-6'} bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 w-full max-sm:overflow-x-hidden`}>
        ...
        <div className="relative h-64 sm:h-72 md:h-80 flex items-stretch">
          ...
          <div className="relative flex justify-around w-full h-full px-2 sm:px-3 md:px-4">
            {tiers.map((tier) => {
              ...
              const isOpponentTier = opponent_turnover_pressure_tier === tier
              ...
              return (
                ...
                max-sm:right-0 sm:left-1/2 sm:transform sm:-translate-x-1/2">
                  ...
                  <div className="mt-2 sm:mt-3 text-center">
                    ...
                    {isOpponentTier && opponent_opp_tov_rank && (
      ...
      <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-700">
        <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 leading-relaxed">
          <strong>How to read:</strong> Shows {team_abbreviation}'s turnovers per game when facing different defensive pressure levels.
          Elite pressure (ranks 1-10) forces most turnovers, while low pressure (ranks 21-30) forces fewest.
          {opponent_turnover_pressure_tier && (
            <span className="block mt-2 text-yellow-600 dark:text-yellow-400 font-medium">
              Today's opponent applies <strong>{opponent_turnover_pressure_tier}</strong> defensive pressure.
            </span>
          )}
        </p>
      </div>
...
export default TurnoverVsDefensePressureChart
